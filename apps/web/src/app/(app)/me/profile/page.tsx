...
const ROLE_LABELS: Record<string, { label: string; tone: string }> = {
  admin: { label: "Quản trị", tone: "bg-rose-50 text-rose-700 ring-rose-200" },
  planner: { label: "Thiết kế", tone: "bg-indigo-50 text-indigo-700 ring-indigo-200" },
  operator: { label: "Gia công", tone: "bg-emerald-50 text-emerald-700 ring-emerald-200" },
  warehouse: { label: "Kho", tone: "bg-amber-50 text-amber-700 ring-amber-200" },
  purchaser: { label: "Thu mua", tone: "bg-sky-50 text-sky-700 ring-sky-200" },
};
    ...
    <div className="flex h-full flex-col overflow-auto bg-zinc-50">
      <header className="border-b border-zinc-200 bg-white px-6 py-4">
        <nav aria-label="Breadcrumb" className="text-xs text-zinc-500">
          ...
          <span className="font-medium text-zinc-900">Hồ sơ cá nhân</span>
        </nav>
        ...
        {session.isLoading ? (
          ...
          <div className="space-y-4">
            ...
            <section className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
              <div className="flex items-center gap-4 bg-gradient-to-br from-indigo-50 to-violet-50/50 p-5">
...
function LogoutCard() {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="group flex w-full items-center gap-3 rounded-xl border border-zinc-200 bg-white p-3 shadow-sm transition-all hover:border-rose-300 hover:shadow-md disabled:opacity-50"
    >
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-rose-50 text-rose-600 ring-1 ring-inset ring-rose-100 group-hover:bg-rose-100">
      ...
      <div className="min-w-0 flex-1 text-left">
        ...
        <div className="text-[11px] text-zinc-500">Kết thúc phiên hiện tại</div>
      </div>
    </button>
  );
}
