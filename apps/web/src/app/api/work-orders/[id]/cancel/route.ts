...
/** POST /api/work-orders/[id]/cancel — any non-final → CANCELLED (admin only). */
export async function POST(
  ...
  try {
    ...
    logger.error({ err, id: params.id }, "cancel WO failed");
    return jsonError("INTERNAL", "Không cancel được WO.", 500);
  }
}
