...
/** POST /api/work-orders/[id]/start — DRAFT/QUEUED → IN_PROGRESS (admin/planner/operator). */
export async function POST(
  req: NextRequest,
  { params }: { params: { id: string } },
) {
  const guard = await requireSession(req, "admin", "planner", "operator");
  ...
  try {
    const wo = await startWO(params.id, body.data.versionLock);
    ...
    logger.error({ err, id: params.id }, "start WO failed");
    return jsonError("INTERNAL", "Không start được WO.", 500);
  }
}
