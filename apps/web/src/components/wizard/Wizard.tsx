...
export function Wizard({
  ...
  return (
    <div className={cn("flex h-full flex-col", className)}>
      ...
      {/* Content */}
      <div className="flex-1 min-h-0 overflow-auto bg-zinc-50 p-6 dark:bg-zinc-800">
...
function WizardProgress({
  ...
  return (
    <ol
      ...
      className="flex flex-wrap items-center gap-1 border-b border-zinc-200 bg-white px-6 py-3 dark:border-zinc-800 dark:bg-zinc-900"
    >
      {steps.map((s, i) => {
          ...
          <React.Fragment key={s.key}>
            <li className="flex-shrink-0">
              <Tag
                ...
                className={cn(
                  ...
                  isActive &&
                    "border-indigo-600 bg-indigo-50 text-indigo-900 shadow-sm dark:border-indigo-500 dark:bg-indigo-950/40 dark:text-indigo-300",
                  !isActive && (isDone || isPast) &&
                    "border-emerald-300 bg-emerald-50 text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-400",
                  !isActive && !(isDone || isPast) &&
                    "border-zinc-200 bg-white text-zinc-500 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-400",
                  onStepClick && "cursor-pointer hover:border-zinc-400 dark:hover:border-zinc-600",
                )}
              >
                <span
                  className={cn(
            ...
            {i < steps.length - 1 ? (
              <li
                aria-hidden="true"
                className={cn(
                  "flex-shrink-0 text-zinc-300 dark:text-zinc-700",
                  i < currentIdx && "text-emerald-400 dark:text-emerald-500",
                )}
              >
                <ChevronRightIcon />
              </li>
    ...
    >
      ...
      <dl className="grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2">
        {items.map((it) => (
          <div
            key={it.label}
            className="flex items-baseline justify-between gap-3 border-b border-zinc-100 py-1.5 last:border-b-0 dark:border-zinc-800"
          >
