...
export function BomProductionPanel({ bomId }: { bomId: string }) {
  ...
  if (!data) return (
    <div className="flex h-full items-center justify-center p-6 text-sm text-zinc-500 dark:text-zinc-400">Không có dữ liệu sản xuất.</div>
  );
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-zinc-200 bg-white px-5 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        {(dateFrom || dateTo) && (
          <button type="button" onClick={() => { setDateFrom(""); setDateTo(""); }}
            className="text-sm text-indigo-600 hover:underline dark:text-indigo-400">Bỏ lọc</button>
        )}
        <span className="ml-auto text-xs font-medium text-zinc-500 dark:text-zinc-400">
          <span className="tabular-nums text-zinc-900 dark:text-zinc-50">{filteredWOs.length}</span> / <span className="tabular-nums">{data.recentWorkOrders.length}</span> WO theo Planned Start
        </span>
      </div>
        ...
        <div className="rounded-xl border border-zinc-200 bg-white overflow-hidden dark:border-zinc-800 dark:bg-zinc-900">
          ...
          {filteredWOs.length === 0 ? (
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {filteredWOs.map((wo) => (
                  <tr key={wo.id} className="group border-b border-zinc-50 transition-colors hover:bg-zinc-50/70 dark:border-zinc-800 dark:hover:bg-zinc-800/60">
                    <td className="px-5 py-3.5">
                      <Link href={`/work-orders/${wo.id}`} className="font-mono text-sm font-bold text-indigo-600 hover:underline dark:text-indigo-400">
                        {wo.woNo}
                      </Link>
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      ...
                      <span className="font-mono text-sm tabular-nums text-zinc-600 dark:text-zinc-400">{formatNumber(wo.plannedQty)}</span>
                    </td>
                    <td className="px-5 py-3.5"><ProgressBar pct={wo.progressPct} /></td>
                    <td className="px-5 py-3.5">
                      <span className="text-sm text-zinc-600 dark:text-zinc-400">{wo.plannedEnd ? formatDate(wo.plannedEnd, "dd/MM/yyyy") : "—"}</span>
                    </td>
