...
const TONE_CLASS: Record<KpiRow["tone"], string> = {
  indigo: "border-indigo-200 bg-indigo-50/40 text-indigo-700 dark:border-indigo-800 dark:bg-indigo-950/40 dark:text-indigo-400",
  emerald: "border-emerald-200 bg-emerald-50/40 text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-400",
  amber: "border-amber-200 bg-amber-50/40 text-amber-700 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-400",
  rose: "border-rose-200 bg-rose-50/40 text-rose-700 dark:border-rose-800 dark:bg-rose-950/40 dark:text-rose-400",
};
  ...
  return (
    <div className="flex flex-col gap-5 p-6">
        ...
        {cards.map((c) => {
          ...
          return (
            <Link
              key={c.key}
              href={c.href}
              className={`group relative flex flex-col gap-2 rounded-lg border bg-white p-4 transition-colors hover:border-zinc-300 hover:shadow-sm dark:border-zinc-700 dark:bg-zinc-900 dark:hover:border-zinc-600`}
            >
              <span
              ...
              <ArrowRight
                className="absolute right-3 top-3 h-3.5 w-3.5 text-zinc-400 transition-transform group-hover:translate-x-0.5 group-hover:text-zinc-600 dark:text-zinc-500 dark:group-hover:text-zinc-300"
                aria-hidden="true"
              />
            </Link>
          );
      ...
      >
        <header className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-100 px-4 py-3 dark:border-zinc-800">
          <div>
            ...
            <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
              ...
              <span className="font-medium text-amber-700 dark:text-amber-400">Reserved</span>.
        ...
        <div className="max-h-[420px] overflow-auto">
          {balanceQuery.isLoading ? (
            ...
            <table className="w-full border-separate border-spacing-0 text-sm">
              <thead className="sticky top-0 z-10 bg-zinc-50 text-[10px] font-medium uppercase tracking-wide text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">
                <tr className="h-8">
                  <th className="border-b border-zinc-200 dark:border-zinc-700 px-3 text-left">
                    SKU
                  </th>
                  <th className="border-b border-zinc-200 dark:border-zinc-700 px-3 text-left">
                    Tên
                  </th>
                  <th className="border-b border-zinc-200 dark:border-zinc-700 px-3 text-center">
                    Đơn vị
                  </th>
                  <th className="border-b border-zinc-200 dark:border-zinc-700 px-3 text-right">
                    On-hand
                  </th>
                  <th
                    className="border-b border-zinc-200 dark:border-zinc-700 px-3 text-right"
              ...
              <tbody>
                {filteredRows.map((r) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-3 py-2 font-mono text-[12px] font-medium text-zinc-900 dark:text-zinc-50">
                        {r.sku}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-zinc-700 dark:text-zinc-300">
                        {r.name}
                      </td>
                      ...
                      <td
                        className={cn(
                          "px-3 py-2 text-right font-mono text-[12px] tabular-nums",
                          r.holdQty > 0 ? "text-rose-700 dark:text-rose-400" : "text-zinc-300 dark:text-zinc-600",
                        )}
                      >
        ...
        <footer className="flex items-center justify-between border-t border-zinc-100 px-4 py-2 text-[11px] text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
          <span>
      ...
      >
        ...
        <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
          ...
          >
            ...
            <span className="text-zinc-500 dark:text-zinc-400">Approve PO khi đủ 95%+</span>
          </Link>
        </div>
      </section>
    </div>
