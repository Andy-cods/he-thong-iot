      ...
      <div className="flex-1 overflow-auto">
        {list.isLoading ? (
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {rows.map((row) => {
                const blank = (row.blankSize ?? {}) as {
                  freeText?: string;
                  scrapPct?: number;
                };
                const sizeText = blank.freeText ?? "";
                const scrapPct =
                ...
                return (
                  <tr key={row.id} className="border-t border-zinc-100 align-top">
                    ...
                    <td className="px-2 py-1">
                      <Input
                        defaultValue={sizeText}
                        onBlur={(e) => {
                          const v = e.target.value.trim();
                          if (v !== sizeText) {
                            // Merge giữ nguyên các field jsonb khác (scrapPct).
                            const next = { ...blank };
                            if (v) next.freeText = v;
                            else delete next.freeText;
                            handlePatch(row.id, { blankSize: next });
                          }
                        }}
                    ...
                    <td className="px-2 py-1 text-right">
                      <Input
                        ...
                        max="100"
                        step="0.5"
                        defaultValue={scrapPct ?? ""}
                        onBlur={(e) => {
                          const raw = e.target.value;
                          const next = { ...blank };
                          if (raw === "") {
                            delete next.scrapPct;
                          } else {
                            const n = Number(raw);
                            if (!Number.isFinite(n) || n < 0 || n > 100) return;
                            next.scrapPct = n;
                          }
                          if (next.scrapPct !== blank.scrapPct) {
                            handlePatch(row.id, { blankSize: next });
                          }
                        }}
                        placeholder="0"
      ...
      <div className="flex-1 overflow-auto">
        {list.isLoading ? (
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {rows.map((row) => {
                // Hiển thị thời gian ở phút cho gần thực tế gia công.
                // DB lưu hoursEstimated (giờ) → convert × 60.
                const minutesValue =
                  row.hoursEstimated !== null
                    ...
                    <td className="px-2 py-1 text-right">
                      <Input
                        ...
                        onBlur={(e) => {
                          const raw = e.target.value;
                          if (raw === "") {
                            if (row.hoursEstimated !== null)
                              handlePatch(row.id, { hoursEstimated: null });
                            return;
                          }
                          const minutes = Number(raw);
                          if (!Number.isFinite(minutes) || minutes < 0) return;
                          const hours = minutes / 60;
                          // Chỉ patch khi giá trị quy đổi khác bản hiện tại.
                          const current = row.hoursEstimated
                            ? Number(row.hoursEstimated)
                            : null;
                          if (current === null || Math.abs(current - hours) > 1e-6) {
                            handlePatch(row.id, { hoursEstimated: hours });
                          }
                        }}
                        placeholder="0"
                        disabled={readOnly}
